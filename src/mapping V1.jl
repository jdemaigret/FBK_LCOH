...
function createmap(gisregion, to_plot, to_mask, regionlist, lons, lats, colors, source, dest, xs, ys,
    ...
    colors = [
        RGBA(0,0,0,0),       # Fondoscala, non mi interessa plottarlo
        RGBA(0.5,0.5,0.5,0), # Grigio, le zone di esclusione
        RGBA(0.4,0.6,0.9,1), # Zona mare
        RGBA(0.3,0.3,0.45,1) # NOREGION
    ]
    ...
    # fig=GLMakie.Figure(resolution = pngsize)

    scene = GLMakie.surface(xs, ys; color=Float16.(to_plot), colormap= :autumn1, colorrange=(colorrange_min,colorrange_max), shading=false, show_axis=false, scale_plot=false, interpolate=false)
    ...
    # Makie.save(filename, fig; resolution=pngsize)
...
function createmap_lcoh(gisregion, to_plot, to_mask, regionlist, lons, lats, colors, source, dest, xs, ys,
    ...
    ymin, ymax = extrema(ys)
    aspect_ratio = (ymax - ymin) / (xmax - xmin)
    pngwidth = 10*round(Int, resolutionscale*1.02*size(to_plot,1)) # allow for margins (+1% on both sides)
    pngsize = pngwidth, round(Int, pngwidth * aspect_ratio)     # use aspect ratio after projection transformation
    ...
    colors_eligible = [
        RGBA(0.5,0.5,0.5,1),  # Fondoscala, sarebbe dove la land NON e' eligible.
        RGBA(0.5,0.5,0.5,0),  # Fine scala, non mi interessa plottaro (sarebbe dove la land e' eligible).
    ]
    ...
    to_plot_filtered = imfilter(to_plot, Kernel.gaussian(2));
    ...
    # eligible[regions .== NOREGION] .= 0;
    eligible_filtered = imfilter(eligible, Kernel.gaussian(2));
    epsilon = 0.000001
    to_plot_final = to_plot_filtered ./ (eligible_filtered .+ epsilon)

    # fig=GLMakie.Figure(resolution = pngsize)

    cs1 = ColorScheme(ColorSchemes.autumn1.colors)
    ...
    N_colors = round(Int, colorrange_min / Δ_palette)
    ...
    scene = GLMakie.surface(xs, ys; color=Float32.(to_plot_final), colormap=colorSchemeNew, cbar=true, colorrange=(colorrange_min, colorrange_max), shading=false, show_axis=false, scale_plot=false, interpolate=false)
    # Float16.(to_plot)
    GLMakie.surface!(scene, xs, ys, to_mask; colormap=colors, cbar=false, shading=false, show_axis=false, scale_plot=false, interpolate=false)

    GLMakie.surface!(scene, xs, ys, eligible; colormap=colors_eligible, cbar=false, shading=false, show_axis=false, scale_plot=false, interpolate=false)
    # cbar = fig[:,2] = Colorbar(fig, scene)
    # scene = surface(xs, ys; color=float.(regions), colormap=colors,
    #                         shading=false, show_axis=false, scale_plot=false, interpolate=false)
    ga = geoaxis!(scene, extrema(lons), extrema(lats); crs=(src=source, dest = dest,))[end]
