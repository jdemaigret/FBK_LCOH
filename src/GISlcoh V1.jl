...
function GISlcoh(; savetodisk=false, plotmasks=false, optionlist...)
   ...
   avg_offshore_dist_1, avg_offshore_dist_2, avg_offshore_dist_3, avg_offshore_dist_4, avg_offshore_dist_5 = calculate_mean_CF(loptions, regions, lonrange, latrange, solarGTI, windspeed, mask_pv_lcoh, mask_onshore_lcoh, mask_offshore_lcoh,pv_system_efficiency)
    ...
    filename0 = in_datafolder("output", "$gisregion results_matrix last.txt");
    filename1 = in_datafolder("output", "$gisregion charachteristic_cf_curves last.txt");
    filename2 = in_datafolder("output", "$gisregion max_and_min_CF last.txt");
    filename3 = in_datafolder("output", "$gisregion Caps CF sums and averages last.txt");
    filename4 = in_datafolder("output", "$gisregion Areas and Potentials last.txt");
...
function calc_LCOH_CAPS_PROD(loptions, regions, solarGTI, offshoreregions, regionlist, mask_pv_lcoh,
    ...
    @time   for j in 1:nlats
                ...
                for i in 1:nlons
                    ...
                    println("typeof(colrange)")
                    println(typeof(colrange))
                    println("colrange")
                    println(colrange)
                    println("typeof(rowrange)")
                    println(typeof(rowrange))
                    println("rowrange")
                    println(rowrange)
                    ...
                    println("typeof(colrange_wo_zero)")
                    println(typeof(colrange_wo_zero))
                    println("colrange_wo_zero")
                    println(colrange_wo_zero)
                    println("typeof(rowrange_wo_zero)")
                    println(typeof(rowrange_wo_zero))
                    println("rowrange_wo_zero")
                    println(rowrange_wo_zero)
...
function calculate_LCOE(gisregion, WACC_value)
    ...
    distributions = readdlm("Characteristic Curves V8\\$gisregion charachteristic_cf_curves V8.txt" ,',', skipstart=1)
