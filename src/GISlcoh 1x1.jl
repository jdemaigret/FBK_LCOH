using DelimitedFiles, JuMP, GLPK, NPFinancial, Ipopt, NLopt, StatsBase, Plots, FLoops
...
function GISlcoh(; savetodisk=false, plotmasks=false, optionlist...)
    ...
    regions, offshoreregions, regionlist, gridaccess, popdens, topo, land, protected, lonrange, latrange, aqueduct =
    ...
    mask_lcoh = create_lcoh_mask(mask_pv_lcoh, mask_onshore_lcoh, mask_offshore_lcoh, aqueduct)
    ...
    plot_lcoh(loptions, regions, mask_lcoh, mask_offshore_lcoh, lonrange, latrange, lcoh_region_matrix, regionlist; plotmasks=plotmasks, downsample=1)
...
#Nuova funzione
function plot_lcoh(loptions, regions, mask_lcoh, mask_offshore_lcoh, lonrange, latrange, lcoh_region_matrix, regionlist; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        to_mask = ones(size(regions))*3;     #Quello che voglio mascherare
...
function calc_LCOH_CAPS_PROD(loptions, regions, solarGTI, offshoreregions, regionlist, mask_pv_lcoh,
    ...
    global contatore     = 0;
    global contatore_j   = 0;
    global contatore_i   = 0;
    ...
    @time   @floop ThreadedEx() for j in 1:nlats
                # contatore_j = contatore_j + 1
                # println("Contatore j $contatore_j")
                eralat = eralats[j]
                colrange = latmap[lat2col(eralat+erares/2, res):lat2col(eralat-erares/2, res)-1]
                for i in 1:nlons
                    # contatore_i = contatore_i + 1
                    # println("Contatore i $contatore_i")
                    #nlons
                    #for i in 5:7
                    eralon = eralons[i]
                    GTI = solarGTI[:, i, j]
                    wind_onshore  = speed2capacityfactor_onshore.(windspeed[:, i, j])
                    wind_offshore = speed2capacityfactor_offshore.(windspeed[:, i, j])#da inrerire power curve in GISwind
                    #CF_pvplant[:,i,j] = GTI
                    rowrange = lonmap[lon2row(eralon-erares/2, res):lon2row(eralon+erares/2, res)-1]
                    for c in colrange
                        for r in rowrange
                            ...
                            if mask_pv_lcoh[r,c]>0 && mask_onshore_lcoh[r,c]>0  #sia pv che onshore
                                ...
                                production_region_matrix[r,c] = find_minimum_LCOH(loptions, lonrange, latrange, regionlist, dim_year_lon_lats,
                                                            GTI, area, wind_onshore, wind_offshore, type_min);
                            end
