...
function GISsolar(; savetodisk=true, plotmasks=false, optionlist...)
    ...
    if savetodisk
        ...
        open(in_datafolder("output", "GISdata_solar$(era_year)_$gisregion$filenamesuffix.csv"), "w") do file
            write(file, string(solardata[1]))
            #CF_pvrooftop[1,1,1]=5.0
            #for i = 1:8760
            #    write(file, string(CF_pvrooftop[i,1,1]))
            #end
            #write(file, string(length(CF_pvrooftop)))
