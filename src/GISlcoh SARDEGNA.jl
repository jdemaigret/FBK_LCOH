...
function GISlcoh(; savetodisk=false, plotmasks=false, optionlist...)
   ...
   avg_offshore_dist_1, avg_offshore_dist_2, avg_offshore_dist_3, avg_offshore_dist_4, avg_offshore_dist_5 = calculate_mean_CF(loptions, regions, lonrange, latrange, solarGTI, windspeed, mask_pv_lcoh, mask_onshore_lcoh, mask_offshore_lcoh,pv_system_efficiency)
    ...
    pv_region_matrix, onshore_region_matrix, offshore_region_matrix, el_region_matrix, lcoh_region_matrix, production_region_matrix, lcoe_nocurt_region_matrix,
    ...
    filename_pv       = in_datafolder("output", "$gisregion pv_matrix (come PLEXOS BWS 1).txt");
    filename_onshore  = in_datafolder("output", "$gisregion onshore_matrix (come PLEXOS BWS 1).txt");
    filename_offshore = in_datafolder("output", "$gisregion offshore_matrix (come PLEXOS BWS 1).txt");
    filename_el       = in_datafolder("output", "$gisregion el_matrix (come PLEXOS BWS 1).txt");
    filename_lcoh     = in_datafolder("output", "$gisregion lcoh_matrix (come PLEXOS BWS 1).txt");
    filename_prod     = in_datafolder("output", "$gisregion prod_matrix (come PLEXOS BWS 1).txt");
    filename_lcoe     = in_datafolder("output", "$gisregion lcoe_matrix (come PLEXOS BWS 1).txt");
    ...
    filename0 = in_datafolder("output", "$gisregion results_matrix (come PLEXOS BWS 1).txt");
    filename1 = in_datafolder("output", "$gisregion charachteristic_cf_curves (come PLEXOS BWS 1).txt");
    filename2 = in_datafolder("output", "$gisregion max_and_min_CF (come PLEXOS BWS 1).txt");
    filename3 = in_datafolder("output", "$gisregion Caps CF sums and average (come PLEXOS BWS 1).txt");
    filename4 = in_datafolder("output", "$gisregion Areas and Potentials (come PLEXOS BWS 1).txt");
    # filename5 = in_datafolder("output", "$gisregion LCOEs (come PLEXOS BWS 1).txt");
    filename7 = in_datafolder("output", "$gisregion GTI (come PLEXOS BWS 1).txt")
    ...
    # plot_lcoe(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, production_region_matrix, regionlist, aqueduct, slope, mask_pv_lcoh, mask_onshore_lcoh; plotmasks=plotmasks, downsample=1)
    # plot_pv(loptions, regions,  mask_pv_lcoh, lonrange, latrange, pv_region_matrix, regionlist, aqueduct, slope, mask_offshore_lcoh; plotmasks=plotmasks, downsample=1)
    ...
    # plot_GTI(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, sumcfpv_region_matrix, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)
    # plot_wind_onshore(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, sumcfonshore_region_matrix, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)
    # plot_wind_offshore(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, sumcfoffshore_region_matrix, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)
...
function plot_lcoh(loptions, regions, mask_offshore_lcoh, lonrange, latrange, lcoh_region_matrix, regionlist, aqueduct, slope, mask_altitude_lcoh, mask_aqueduct_lcoh, mask_pv_lcoh, mask_onshore_lcoh, solarGTI; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        println("Reading LCOH matrix...")
        lcoh_region_matrix = readdlm("C:\\Users\\jdemaigret\\GISData\\Matrices\\Sardegna\\$gisregion lcoh_matrix (come PLEXOS BWS 1).txt",',')
        ...
        println("...saving lcoh range...")
        filename0 = in_datafolder("output", "$gisregion range lcoh $(lcoh_threshold).txt")
        isfile(filename0) && rm(filename0);
        writedlm(filename0, [minimum(filter(!isequal(0), to_plot))  maximum(to_plot)] , '\t');

        scene = createmap_lcoh("$(gisregion)_lcoh_(come PLEXOS BWS) $(lcoh_threshold)$filenamesuffix", to_plot, to_mask, regionlist, lons, lats, [], source, dest, xs, ys,
...
function plot_lcoe(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, production_region_matrix, regionlist, aqueduct, slope, mask_pv_lcoh, mask_onshore_lcoh; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        lcoh_region_matrix = readdlm("C:\\Users\\jdemaigret\\GISData\\Matrices\\Sardegna\\$gisregion lcoe_matrix.txt",',')
        # mask_pv_lcoh       = readdlm(in_datafolder("output" ,"$gisregion mask_pv_matrix (prova plot).txt") ,',')
        # mask_onshore_lcoh  = readdlm(in_datafolder("output" ,"$gisregion mask_onshore_matrix (prova plot).txt") ,',')
        # mask_offshore_lcoh = readdlm(in_datafolder("output" ,"$gisregion mask_offshore_matrix (prova plot).txt") ,',')
        ...
        println("...saving production range...")
        filename0 = in_datafolder("output", "$gisregion range production.txt")
        isfile(filename0) && rm(filename0);
        writedlm(filename0, [minimum(filter(!isequal(0), to_plot))  maximum(to_plot)] , '\t');

        scene = createmap("$(gisregion)_lcoe_ORI$filenamesuffix", to_plot, to_mask, regionlist, lons, lats, [], source, dest, xs, ys,
             [], [], [], [],eligible, lines=false, labels=false, resolutionscale=1, textscale=1, legend=false)
    end
    nothing
end
function plot_pv(loptions, regions,  mask_pv_lcoh, lonrange, latrange, pv_region_matrix, regionlist, aqueduct, slope, mask_offshore_lcoh, windatlas; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        pv_region_matrix = readdlm("C:\\Users\\jdemaigret\\GISData\\Matrices\\Sardegna\\$gisregion pv_matrix (come PLEXOS BWS 1).txt",',')
        # mask_pv_lcoh       = readdlm(in_datafolder("output" ,"$gisregion mask_pv_matrix (prova plot).txt") ,',')
        ...
        scene = createmap("$(gisregion)_pv_ORI$filenamesuffix", to_plot, to_mask, regionlist, lons, lats, [], source, dest, xs, ys,
             [], [], [], [],eligible, lines=false, labels=false, resolutionscale=1, textscale=1, legend=false)
    end
    nothing
end
function plot_onshore(loptions, regions,  mask_onshore_lcoh, lonrange, latrange, onshore_region_matrix, regionlist, aqueduct, slope, mask_offshore_lcoh; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        onshore_region_matrix = readdlm("C:\\Users\\jdemaigret\\GISData\\Matrices\\Sardegna\\$gisregion onshore_matrix.txt",',')
        # mask_onshore_lcoh  = readdlm(in_datafolder("output" ,"$gisregion mask_onshore_matrix (prova plot).txt") ,',')
        ...
        scene = createmap("$(gisregion)_onshore_ORI$filenamesuffix", to_plot, to_mask, regionlist, lons, lats, [], source, dest, xs, ys,
             [], [], [], [],eligible, lines=false, labels=false, resolutionscale=1, textscale=1, legend=false)
    end
    nothing
end
function plot_offshore(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, offshore_region_matrix, regionlist, aqueduct, slope; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        println("Reading Offshore capacity matrix...")
        offshore_region_matrix = readdlm("C:\\Users\\jdemaigret\\GISData\\Matrices\\Sardegna\\$gisregion offshore_matrix.txt",',')
        # mask_offshore_lcoh = readdlm(in_datafolder("output" ,"$gisregion mask_offshore_matrix (prova plot).txt") ,',')
        ...
        scene = createmap("$(gisregion)_offshore_ORI$filenamesuffix", to_plot, to_mask, regionlist, lons, lats, [], source, dest, xs, ys,
             [], [], [], [],eligible, lines=false, labels=false, resolutionscale=1, textscale=1, legend=false)
    end
    nothing
end
function plot_el(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, el_region_matrix, regionlist, aqueduct, slope, mask_pv_lcoh, mask_onshore_lcoh; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        println("Reading Offshore capacity matrix...")
        el_region_matrix = readdlm("C:\\Users\\jdemaigret\\GISData\\Matrices\\Sardegna\\$gisregion el_matrix.txt",',')
        # mask_pv_lcoh       = readdlm(in_datafolder("output" ,"$gisregion mask_pv_matrix (prova plot).txt") ,',')
        # mask_onshore_lcoh  = readdlm(in_datafolder("output" ,"$gisregion mask_onshore_matrix (prova plot).txt") ,',')
        # mask_offshore_lcoh = readdlm(in_datafolder("output" ,"$gisregion mask_offshore_matrix (prova plot).txt") ,',')
        ...
        scene = createmap("$(gisregion)_el_ORI$filenamesuffix", to_plot, to_mask, regionlist, lons, lats, [], source, dest, xs, ys,
             [], [], [], [],eligible, lines=false, labels=false, resolutionscale=1, textscale=1, legend=false)
    end
    nothing
end
function plot_prod(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, production_region_matrix, regionlist, aqueduct, slope, mask_pv_lcoh, mask_onshore_lcoh; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        println("Reading Offshore capacity matrix...")
        production_region_matrix = readdlm("C:\\Users\\jdemaigret\\GISData\\Matrices\\Sardegna\\$gisregion prod_matrix.txt",',')
        # mask_pv_lcoh       = readdlm(in_datafolder("output" ,"$gisregion mask_pv_matrix (prova plot).txt") ,',')
        # mask_onshore_lcoh  = readdlm(in_datafolder("output" ,"$gisregion mask_onshore_matrix (prova plot).txt") ,',')
        # mask_offshore_lcoh = readdlm(in_datafolder("output" ,"$gisregion mask_offshore_matrix (prova plot).txt") ,',')
        ...
        scene = createmap("$(gisregion)_prod_ORI$filenamesuffix", to_plot, to_mask, regionlist, lons, lats, [], source, dest, xs, ys,
             [], [], [], [],eligible, lines=false, labels=false, resolutionscale=1, textscale=1, legend=false)
    end
    nothing
end
...
function plot_GTI(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, sumcfpv_region_matrix, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        to_plot = sumcfpv_region_matrix;       #Quello che voglio plottare
...
function plot_wind_onshore(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, sumcfonshore_region_matrix, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        to_plot = sumcfonshore_region_matrix;       #Quello che voglio plottare
        to_mask = ones(size(regions))*4;     #Quello che voglio mascherare
...
function plot_wind_offshore(loptions, regions,  mask_offshore_lcoh, lonrange, latrange, sumcfoffshore_region_matrix, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)
    ...
    if plotmasks != false   # can == :onlymasks as well
        ...
        to_plot = sumcfoffshore_region_matrix;       #Quello che voglio plottare
        to_mask = ones(size(regions))*4;     #Quello che voglio mascherare
...
function calc_LCOH_CAPS_PROD(loptions, regions, solarGTI, offshoreregions, regionlist, mask_pv_lcoh,
    ...
    CAPEX_pv               = 326.8756674
    CAPEX_wind_onshore     = 994.3793507
    CAPEX_wind_offshore    = 1796.954236
    # CAPEX_wind_offshore    = 2431.772823
    ...
    global contatore = 0;
    num_tot_pixel = size(regions)[1]*size(regions)[2]

    updateprogress = Progress(nlats, 1)

    @time for j in 1:1 #nlats
        ...
        rowrange = lonmap[lon2row(eralon-erares/2, res):lon2row(eralon+erares/2, res)-1]
            ...
            for c in colrange
                for r in rowrange
                    ...
                    prop_coeff = windatlas[r,c] / mean(windspeed[:, i, j])

                    rescaled_windspeed = prop_coeff .* windpseed[:, i, j];

                    wind_onshore  = speed2capacityfactor_onshore.(rescaled_windspeed)
                    wind_offshore = speed2capacityfactor_offshore.(rescaled_windspeed)

                    if hybrid_system_mask[r,c] != 0

                        pv_region_matrix[r,c],
                        onshore_region_matrix[r,c],
                        offshore_region_matrix[r,c],
                        el_region_matrix[r,c],
                        lcoh_region_matrix[r,c],
                        production_region_matrix[r,c],
                        lcoe_nocurt_region_matrix[r,c] = find_minimum_LCOH(loptions, lonrange, latrange, regionlist, dim_year_lon_lats,
                        ...
                        sumcfonshore_region_matrix[r,c]  = sum(wind_onshore);
                        sumcfoffshore_region_matrix[r,c] = sum(wind_offshore);

                    end

                    if (hybrid_system_mask[r,c] == 0) && (mask_pv_lcoh[r,c] != 0)
                    ...
                    if (hybrid_system_mask[r,c] == 0) && (mask_onshore_lcoh[r,c] != 0)
    ...
    return pv_region_matrix, onshore_region_matrix, offshore_region_matrix, el_region_matrix, lcoh_region_matrix, production_region_matrix, lcoe_nocurt_region_matrix,
