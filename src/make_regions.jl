export GADM, NUTS, makeregions, makeregions_nuts, makeoffshoreregions, saveregions, loadregions,
        saveregions_global, subregions, all_gadm_subregions, read_save_region
...
function read_save_region()
    ...
    composed_region = excel_file["Region Definition"]
...
function makeregions(regiondefinitionarray; allowmixed=false)
    regionnames, nutsdef, gadmdef = splitregiondefinitions(regiondefinitionarray)   #Mi ritorna regionnames=(GADM{String}(["Italy"], ("Liguria",)),)
                                                                                    #e gadmdef=Tuple{GADM{String}}[(GADM{String}(["Italy"], ("Liguria",)),)]
                                                                                    # gadmsef serve solo a dire se sono in gadm o nuts
    ...
    region = zeros(Int16, (36000,18000))    # hard code size for now, crea una matrice della dimensione del tiff di gadm piena di zeri,
                                            #la popola con makeregions_gadm!
    if use_nuts
        nuts, subregionnames = read_nuts()
        makeregions_nuts!(region, nuts, subregionnames, nutsdef)
    end
    if use_gadm
        gadm, subregionnames = read_gadm()  #legge il csv di gadmfields per i nomi, legge il tif per gadm, ogni pixel di gadm ha il codice uid
...
function makeregions_gadm!(region, gadm, subregionnames, regiondefinitions)
    ...
    regionlookup = build_inverseregionlookup(regiondefinitions) #ritorna un Dict(("Italy", "Liguria", "*") => 1)
    ...
    for c in randperm(cols)
        for r = 1:rows
            ...
            regid = lookup_regionnames(regionlookup, reg0, reg1, reg2)              #entra nel Dictionary e mi ritorna "Italy", "Liguria", "*"
            if regid > 0
                region[r,c] = regid
            end
        end
...
function lookup_regionnames(regionlookup, reg0, reg1, reg2) # La funzione get() accede agli elementi del Dictionary
                                                            # Sintassi get(Dictionary_name, Key_name, Default Value)
    v = get(regionlookup, (reg0, "*", "*"), 0)              # regionlookup sarebbe Dict(("Italy", "Liguria", "*") => 1)
    v > 0 && return v
    v = get(regionlookup, (reg0, reg1, "*"), 0)             #il get ritorna l'elemento del Dictionary: "Italy", "Liguria", "*"
    v > 0 && return v
