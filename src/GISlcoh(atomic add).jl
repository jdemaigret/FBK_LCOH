using DelimitedFiles, StatsBase, Plots, FLoops, Statistics, Base.Threads
...
function GISlcoh(; savetodisk=false, plotmasks=false, optionlist...)
   ...
   avg_offshore_dist_1, avg_offshore_dist_2, avg_offshore_dist_3, avg_offshore_dist_4, avg_offshore_dist_5 = calculate_mean_CF(loptions, regions, lonrange, latrange, solarGTI, windspeed, mask_pv_lcoh, mask_onshore_lcoh, mask_offshore_lcoh)
    ...
    filename0 = in_datafolder("output", "$gisregion results_matrix V9.txt");
    filename1 = in_datafolder("output", "$gisregion charachteristic_cf_curves V9.txt");
    filename2 = in_datafolder("output", "$gisregion max_and_min_CF V9.txt");
    filename3 = in_datafolder("output", "$gisregion Caps CF sums and averages V9.txt");
    filename4 = in_datafolder("output", "$gisregion Areas and Potentials V9.txt");
    # filename5 = in_datafolder("output", "$gisregion LCOEs V9.txt");
...
function calculate_mean_CF(loptions, regions, lonrange, latrange, solarGTI, windspeed, mask_pv_lcoh, mask_onshore_lcoh, mask_offshore_lcoh)
    ...
    pv_cap_1 = Threads.Atomic{Float64}(0.0)
    pv_cap_2 = Threads.Atomic{Float64}(0.0)
    pv_cap_3 = Threads.Atomic{Float64}(0.0)
    pv_cap_4 = Threads.Atomic{Float64}(0.0)
    pv_cap_5 = Threads.Atomic{Float64}(0.0)
    ...
    onshore_cap_1  = Threads.Atomic{Float64}(0.0)
    onshore_cap_2  = Threads.Atomic{Float64}(0.0)
    onshore_cap_3  = Threads.Atomic{Float64}(0.0)
    onshore_cap_4  = Threads.Atomic{Float64}(0.0)
    onshore_cap_5  = Threads.Atomic{Float64}(0.0)
    ...
    offshore_cap_1  = Threads.Atomic{Float64}(0.0)
    offshore_cap_2  = Threads.Atomic{Float64}(0.0)
    offshore_cap_3  = Threads.Atomic{Float64}(0.0)
    offshore_cap_4  = Threads.Atomic{Float64}(0.0)
    offshore_cap_5  = Threads.Atomic{Float64}(0.0)
    ...
    counter_pv_dist_1 = Threads.Atomic{Int64}(0)
    counter_pv_dist_2 = Threads.Atomic{Int64}(0)
    counter_pv_dist_3 = Threads.Atomic{Int64}(0)
    counter_pv_dist_4 = Threads.Atomic{Int64}(0)
    counter_pv_dist_5 = Threads.Atomic{Int64}(0)
    counter_onshore_dist_1 = Threads.Atomic{Int64}(0)
    counter_onshore_dist_2 = Threads.Atomic{Int64}(0)
    counter_onshore_dist_3 = Threads.Atomic{Int64}(0)
    counter_onshore_dist_4 = Threads.Atomic{Int64}(0)
    counter_onshore_dist_5 = Threads.Atomic{Int64}(0)
    counter_offshore_dist_1 = Threads.Atomic{Int64}(0)
    counter_offshore_dist_2 = Threads.Atomic{Int64}(0)
    counter_offshore_dist_3 = Threads.Atomic{Int64}(0)
    counter_offshore_dist_4 = Threads.Atomic{Int64}(0)
    counter_offshore_dist_5 = Threads.Atomic{Int64}(0)
            ...
            for c in colrange
                for r in rowrange
                    ...
                    if mask_pv_lcoh[r,c]>0
                        ...
                        if cf_pv_matrix[r,c]>0.26*system_losses

                            for id in 1:8760
                                t0 = Threads.Atomic{Float32}(pv_dist_1[id])
                                Threads.atomic_add!(t0, solarGTI[id,i,j])
                                pv_dist_1[id]=t0[]
                            end
                            Threads.atomic_add!(counter_pv_dist_1, 1)
                            Threads.atomic_add!(pv_cap_1, cellarea[c] * pv_density * plant_area)

                        end

                        if cf_pv_matrix[r,c]>0.22*system_losses && cf_pv_matrix[r,c]<=0.26*system_losses

                            for id in 1:8760
                                t0 = Threads.Atomic{Float32}(pv_dist_2[id])
                                Threads.atomic_add!(t0, solarGTI[id,i,j])
                                pv_dist_2[id]=t0[]
                            end
                            Threads.atomic_add!(counter_pv_dist_2, 1)
                            Threads.atomic_add!(pv_cap_2, cellarea[c] * pv_density * plant_area)

                        end

                        if cf_pv_matrix[r,c]>0.18*system_losses && cf_pv_matrix[r,c]<=0.22*system_losses

                            for id in 1:8760
                                t0 = Threads.Atomic{Float32}(pv_dist_3[id])
                                Threads.atomic_add!(t0, solarGTI[id,i,j])
                                pv_dist_3[id]=t0[]
                            end
                            Threads.atomic_add!(counter_pv_dist_3, 1)
                            Threads.atomic_add!(pv_cap_3, cellarea[c] * pv_density * plant_area)

                        end

                        if cf_pv_matrix[r,c]>0.14*system_losses && cf_pv_matrix[r,c]<=0.18*system_losses

                            for id in 1:8760
                                t0 = Threads.Atomic{Float32}(pv_dist_4[id])
                                Threads.atomic_add!(t0, solarGTI[id,i,j])
                                pv_dist_4[id]=t0[]
                            end
                            Threads.atomic_add!(counter_pv_dist_4, 1)
                            Threads.atomic_add!(pv_cap_4, cellarea[c] * pv_density * plant_area)

                        end

                        if cf_pv_matrix[r,c]>0*system_losses && cf_pv_matrix[r,c]<=0.14*system_losses

                            for id in 1:8760
                                t0 = Threads.Atomic{Float32}(pv_dist_5[id])
                                Threads.atomic_add!(t0, solarGTI[id,i,j])
                                pv_dist_5[id]=t0[]
                            end
                            Threads.atomic_add!(counter_pv_dist_5, 1)
                            Threads.atomic_add!(pv_cap_5, cellarea[c] * pv_density * plant_area)
                    ...
                    if  mask_onshore_lcoh[r,c]>0
                        ...
                        if cf_onshore_matrix[r,c]>0.60
                            ...
                            for id in 1:8760
                                t0 = Threads.Atomic{Float64}(onshore_dist_1[id])
                                Threads.atomic_add!(t0, speed2capacityfactor_onshore.(windspeed[id,i,j]))
                                onshore_dist_1[id]=t0[]
                            end
                            Threads.atomic_add!(counter_onshore_dist_1, 1)
                            Threads.atomic_add!(onshore_cap_1, cellarea[c] * onshore_density * area_onshore)
                        ...
                        if cf_onshore_matrix[r,c]>0.45 && cf_onshore_matrix[r,c]<=0.60
                            ...
                            for id in 1:8760
                                t0 = Threads.Atomic{Float64}(onshore_dist_2[id])
                                Threads.atomic_add!(t0, speed2capacityfactor_onshore.(windspeed[id,i,j]))
                                onshore_dist_2[id]=t0[]
                            end
                            Threads.atomic_add!(counter_onshore_dist_2, 1)
                            Threads.atomic_add!(onshore_cap_2, cellarea[c] * onshore_density * area_onshore)
                        ...
                        if cf_onshore_matrix[r,c]>0.30 && cf_onshore_matrix[r,c]<=0.45
                            ...
                            for id in 1:8760
                                t0 = Threads.Atomic{Float64}(onshore_dist_3[id])
                                Threads.atomic_add!(t0, speed2capacityfactor_onshore.(windspeed[id,i,j]))
                                onshore_dist_3[id]=t0[]
                            end
                            Threads.atomic_add!(counter_onshore_dist_3, 1)
                            Threads.atomic_add!(onshore_cap_3, cellarea[c] * onshore_density * area_onshore)
                        ...
                        if cf_onshore_matrix[r,c]>0.15 && cf_onshore_matrix[r,c]<0.30
                            ...
                            for id in 1:8760
                                t0 = Threads.Atomic{Float64}(onshore_dist_4[id])
                                Threads.atomic_add!(t0, speed2capacityfactor_onshore.(windspeed[id,i,j]))
                                onshore_dist_4[id]=t0[]
                            end
                            Threads.atomic_add!(counter_onshore_dist_4, 1)
                            Threads.atomic_add!(onshore_cap_4, cellarea[c] * onshore_density * area_onshore)
                        ...
                        if cf_onshore_matrix[r,c]>0 && cf_onshore_matrix[r,c]<=0.15
                            ...
                            for id in 1:8760
                                t0 = Threads.Atomic{Float64}(onshore_dist_5[id])
                                Threads.atomic_add!(t0, speed2capacityfactor_onshore.(windspeed[id,i,j]))
                                onshore_dist_5[id]=t0[]
                            end
                            Threads.atomic_add!(counter_onshore_dist_5, 1)
                            Threads.atomic_add!(onshore_cap_5, cellarea[c] * onshore_density * area_onshore)
                    ...
                    if mask_offshore_lcoh[r,c]>0
                        ...
                        if cf_offshore_matrix[r,c]>0.6
                            ...
                            for id in 1:8760
                                t0 = Threads.Atomic{Float64}(offshore_dist_1[id])
                                Threads.atomic_add!(t0, speed2capacityfactor_offshore.(windspeed[id,i,j]))
                                offshore_dist_1[id]=t0[]
                            end
                            Threads.atomic_add!(counter_offshore_dist_1, 1)
                            Threads.atomic_add!(offshore_cap_1, cellarea[c] * offshore_density * area_offshore)
                        ...
                        if cf_offshore_matrix[r,c]>0.45 && cf_offshore_matrix[r,c]<=0.60
                            ...
                            for id in 1:8760
                                t0 = Threads.Atomic{Float64}(offshore_dist_2[id])
                                Threads.atomic_add!(t0, speed2capacityfactor_offshore.(windspeed[id,i,j]))
                                offshore_dist_2[id]=t0[]
                            end
                            Threads.atomic_add!(counter_offshore_dist_2, 1)
                            Threads.atomic_add!(offshore_cap_2, cellarea[c] * offshore_density * area_offshore)
                        ...
                        if cf_offshore_matrix[r,c]>0.30 && cf_offshore_matrix[r,c]<=0.45
                            ...
                            for id in 1:8760
                                t0 = Threads.Atomic{Float64}(offshore_dist_3[id])
                                Threads.atomic_add!(t0, speed2capacityfactor_offshore.(windspeed[id,i,j]))
                                offshore_dist_3[id]=t0[]
                            end
                            Threads.atomic_add!(counter_offshore_dist_3, 1)
                            Threads.atomic_add!(offshore_cap_3, cellarea[c] * offshore_density * area_offshore)
                        ...
                        if cf_offshore_matrix[r,c]>0.15 && cf_offshore_matrix[r,c]<0.30
                            ...
                            for id in 1:8760
                                t0 = Threads.Atomic{Float64}(offshore_dist_4[id])
                                Threads.atomic_add!(t0, speed2capacityfactor_offshore.(windspeed[id,i,j]))
                                offshore_dist_4[id]=t0[]
                            end
                            Threads.atomic_add!(counter_offshore_dist_4, 1)
                            Threads.atomic_add!(offshore_cap_4, cellarea[c] * offshore_density * area_offshore)
                        ...
                        if cf_offshore_matrix[r,c]>0 && cf_offshore_matrix[r,c]<=0.15
                            ...
                            for id in 1:8760
                                t0 = Threads.Atomic{Float64}(offshore_dist_5[id])
                                Threads.atomic_add!(t0, speed2capacityfactor_offshore.(windspeed[id,i,j]))
                                offshore_dist_5[id]=t0[]
                            end
                            Threads.atomic_add!(counter_offshore_dist_5, 1)
                            Threads.atomic_add!(offshore_cap_5, cellarea[c] * offshore_density * area_offshore)
...
function calculate_LCOE(gisregion, WACC_value)
    ...
    distributions = readdlm("Characteristic Cruves V8\\$gisregion charachteristic_cf_curves V8.txt" ,',', skipstart=1)
    ...
    CAPEX_and_WACC = readdlm("WACC_CAPEX Versione 4.csv" ,',')
    ...
    #---------------------------------------------------------------------------
        ...
        for t in 1:lifetime_pv
            annual_cost_pv_1[t] = ((CAPEX_pv/lifetime_pv) + (OPEX_pv*CAPEX_pv)) / (1+WACC_pv)^t ;
        ...
        for t in 1:lifetime_pv
            annual_cost_pv_2[t] = ((CAPEX_pv/lifetime_pv) + (OPEX_pv*CAPEX_pv)) / (1+WACC_pv)^t ;
        ...
        for t in 1:lifetime_pv
            annual_cost_pv_3[t] = ((CAPEX_pv/lifetime_pv) + (OPEX_pv*CAPEX_pv)) / (1+WACC_pv)^t ;
        ...
        for t in 1:lifetime_pv
            annual_cost_pv_4[t] = ((CAPEX_pv/lifetime_pv) + (OPEX_pv*CAPEX_pv)) / (1+WACC_pv)^t ;
        ...
        for t in 1:lifetime_pv
            annual_cost_pv_5[t] = ((CAPEX_pv/lifetime_pv) + (OPEX_pv*CAPEX_pv)) / (1+WACC_pv)^t ;
        ...
        #---------------------------------------------------------------------------
        annual_cost_onshore_1 = zeros(1:lifetime_wind)
        annual_ee_prod_onshore_1 = zeros(1:lifetime_wind)
        for t in 1:lifetime_wind
            annual_cost_onshore_1[t] = ((CAPEX_wind_onshore/lifetime_wind) + (OPEX_wind*CAPEX_wind_onshore)) / (1+WACC_onshore)^t ;
        ...
        annual_cost_onshore_2 = zeros(1:lifetime_wind)
        annual_ee_prod_onshore_2 = zeros(1:lifetime_wind)
        for t in 1:lifetime_wind
            annual_cost_onshore_2[t] = ((CAPEX_wind_onshore/lifetime_wind) + (OPEX_wind*CAPEX_wind_onshore)) / (1+WACC_onshore)^t ;
        ...
        annual_cost_onshore_3 = zeros(1:lifetime_wind)
        annual_ee_prod_onshore_3 = zeros(1:lifetime_wind)
        for t in 1:lifetime_wind
            annual_cost_onshore_3[t] = ((CAPEX_wind_onshore/lifetime_wind) + (OPEX_wind*CAPEX_wind_onshore)) / (1+WACC_onshore)^t ;
        ...
        annual_cost_onshore_4 = zeros(1:lifetime_wind)
        annual_ee_prod_onshore_4 = zeros(1:lifetime_wind)
        for t in 1:lifetime_wind
            annual_cost_onshore_4[t] = ((CAPEX_wind_onshore/lifetime_wind) + (OPEX_wind*CAPEX_wind_onshore)) / (1+WACC_onshore)^t ;
        ...
        annual_cost_onshore_5 = zeros(1:lifetime_wind)
        annual_ee_prod_onshore_5 = zeros(1:lifetime_wind)
        for t in 1:lifetime_wind
            annual_cost_onshore_5[t] = ((CAPEX_wind_onshore/lifetime_wind) + (OPEX_wind*CAPEX_wind_onshore)) / (1+WACC_onshore)^t ;
        ...
        #---------------------------------------------------------------------------
        annual_cost_offshore_1 = zeros(1:lifetime_wind)
        annual_ee_prod_offshore_1 = zeros(1:lifetime_wind)
        for t in 1:lifetime_wind
            annual_cost_offshore_1[t] = ((CAPEX_wind_offshore/lifetime_wind) + (OPEX_wind*CAPEX_wind_offshore)) / (1+WACC_offshore)^t ;
        ...
        annual_cost_offshore_2 = zeros(1:lifetime_wind)
        annual_ee_prod_offshore_2 = zeros(1:lifetime_wind)
        for t in 1:lifetime_wind
            annual_cost_offshore_2[t] = ((CAPEX_wind_offshore/lifetime_wind) + (OPEX_wind*CAPEX_wind_offshore)) / (1+WACC_offshore)^t ;
        ...
        annual_cost_offshore_3 = zeros(1:lifetime_wind)
        annual_ee_prod_offshore_3 = zeros(1:lifetime_wind)
        for t in 1:lifetime_wind
            annual_cost_offshore_3[t] = ((CAPEX_wind_offshore/lifetime_wind) + (OPEX_wind*CAPEX_wind_offshore)) / (1+WACC_offshore)^t ;
        ...
        annual_cost_offshore_4 = zeros(1:lifetime_wind)
        annual_ee_prod_offshore_4 = zeros(1:lifetime_wind)
        for t in 1:lifetime_wind
            annual_cost_offshore_4[t] = ((CAPEX_wind_offshore/lifetime_wind) + (OPEX_wind*CAPEX_wind_offshore)) / (1+WACC_offshore)^t ;
        ...
        annual_cost_offshore_5 = zeros(1:lifetime_wind)
        annual_ee_prod_offshore_5 = zeros(1:lifetime_wind)
        for t in 1:lifetime_wind
            annual_cost_offshore_5[t] = ((CAPEX_wind_offshore/lifetime_wind) + (OPEX_wind*CAPEX_wind_offshore)) / (1+WACC_offshore)^t ;
