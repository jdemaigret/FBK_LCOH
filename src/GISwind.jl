...
function read_datasets(options)
    ...
    aqueduct = JLD.load(in_datafolder("aqueduct.jld"), "aqueduct")[lonrange,latrange]
    aqueduct30 = JLD.load(in_datafolder("aqueduct30.jld"), "aqueduct30")[lonrange,latrange]
    slope = JLD.load(in_datafolder("slope.jld"), "slope")[lonrange,latrange]

    popdens = pop ./ cellarea'

    # drawmap(log.(pop))

    return regions, offshoreregions, regionlist, gridaccess, popdens, topo, land, protected, lonrange, latrange, aqueduct, slope, aqueduct30
end
...
const windparkcurve_onshore = [
    0.0, 0.0, 0.0, 0.01, 0.04, 0.08, 0.14, 0.25, 0.38, 0.54,
    0.71, 0.82, 0.85, 0.85, 0.85, 0.85, 0.85, 0.85, 0.85, 0.85,
    0.85, 0.85, 0.85, 0.85, 0.85, 0.85, 0.85, 0, 0, 0, 0
]

function speed2capacityfactor_onshore(windspeed)
    if windspeed >= 29 || windspeed < 0
        return 0.0
    end
    fw = floor(Int, windspeed)
    frac = windspeed - fw
    return (1-frac).*windparkcurve_onshore[fw+1] + frac.*windparkcurve_onshore[ceil(Int, windspeed)+1]
end

const windparkcurve_offshore = [
    0.0, 0.0, 0.0, 0.0, 0.02, 0.06, 0.12, 0.20, 0.30, 0.42,
    0.58, 0.77, 0.85, 0.85, 0.85, 0.85, 0.85, 0.85, 0.85, 0.85,
    0.85, 0.85, 0.85, 0.85, 0.85, 0.85, 0.0, 0,0, 0.0, 0.0
]

function speed2capacityfactor_offshore(windspeed)
    if windspeed >= 29 || windspeed < 0
        return 0.0
    end
    fw = floor(Int, windspeed)
    frac = windspeed - fw
    return (1-frac).*windparkcurve_offshore[fw+1] + frac.*windparkcurve_offshore[ceil(Int, windspeed)+1]
end
