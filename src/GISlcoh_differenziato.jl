...
function GISlcoh(; savetodisk=false, plotmasks=false, optionlist...)
    ...
    sumcfpv_region_matrix, sumcfonshore_region_matrix, sumcfoffshore_region_matrix = calc_LCOH_CAPS_PROD(loptions, regions, solarGTI, offshoreregions, regionlist, mask_pv_lcoh,
    ...
    for i in 1:length(FLOH_vector)
        FLOH_vector[i] = vectorized_prod_matrix_wo_0[i]/(8760*vectorized_el_matrix_wo_0[i]/H2_energy_consumption);
        ...
        CURTAILMENT_vector[i] = (ENERGY_PROD_vector[i]-ELECTROLYZER_CONS_vector[i])/ENERGY_PROD_vector[i];
    end
    ...
    header = ["LCOH" "Prod" "PV" "Onshore" "Offshore" "Electrolyzer" "FLOH" "Sum_CF_PV" "Sum_CF_Onshore" "Sum_CF_Offshore" "Curtailment"]
...
function find_minimum_LCOH(loptions, lonrange, latrange, regionlist, dim_year_lon_lats,
    ...
    CAPEX_el               = 250;
    CAPEX_pv               = 299;
    CAPEX_wind_onshore     = 982;
    CAPEX_wind_offshore    = 1827;
    WACC_el                = 0.08
    # WACC_pv                = 0.0543
    # WACC_onshore           = 0.0647
    ...
    elseif type_min == 2
        ...
        z_min_LCOH_rescaled,
        minLCOH,
        rescaled_annual_hydrogen_production = iterative_find_min_2var_pv(loptions, CF_pvplant, max_capacity_pv, CAPEX_pv, CAPEX_el, WACC_el, WACC_pv)
    ...
    elseif type_min == 3
        ...
        z_min_LCOH_rescaled,
        minLCOH,
        rescaled_annual_hydrogen_production = iterative_find_min_2var_onshore(loptions, CF_onshore, max_capacity_onshore, CAPEX_wind_onshore, CAPEX_el, WACC_el, WACC_onshore)
    ...
    elseif type_min == 4
        ...
        z_min_LCOH_rescaled,
        minLCOH,
        rescaled_annual_hydrogen_production = iterative_find_min_2var_offshore(loptions, CF_offshore, max_capacity_offshore, CAPEX_wind_onshore, CAPEX_el, WACC_el, WACC_offshore)
...
#Funzione nuova
function iterative_find_min_2var_pv(loptions, CF_pvplant, max_capacity_pv, CAPEX_pv, CAPEX_el, WACC_el, WACC_pv)
    ...
    global startvalue_pv    = 0;
    global endvalue_pv      = max_cap_pv;
    global startvalue_el_pv = 1;
    global endvalue_el_pv   = max_cap_pv;

    dd_pv=(7,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4);

    for l in 1:length(dd_pv)
        d_pv=dd_pv[l];

        x    = collect(range(startvalue_pv,    length=d_pv, stop=endvalue_pv));
        z_pv = collect(range(startvalue_el_pv, length=d_pv, stop=endvalue_el_pv));

        annual_invest_LCOE_pv         = zeros(length(x));
        annual_energy_production_pv   = zeros(length(x));
        annual_invest_LCOH_pv         = zeros(length(x),length(z_pv));
        annual_hydrogen_production_pv = zeros(length(x),length(z_pv));

        sumCF_pv=sum(CF_pvplant);

        for i in 1:length(x)
	        annual_invest_LCOE_pv[i]       = cost_pv  * x[i];
	        annual_energy_production_pv[i] = sumCF_pv * x[i];
		        for k in 1:length(z_pv)
	                annual_invest_LCOH_pv[i,k] = annual_invest_LCOE_pv[i] + cost_el * z_pv[k];
                        for t in 1:8760
                            annual_hydrogen_production_pv[i,k] += min((CF_pvplant[t]*x[i]),z_pv[k]) / H2_energy_consumption;
                        end
                end
        end

        LCOE_pv = 1000 * annual_invest_LCOE_pv ./ annual_energy_production_pv;
        LCOH_pv = annual_invest_LCOH_pv./annual_hydrogen_production_pv;
        LCOE_min_pv , indicesLCOEmin_pv = findmin(LCOE_pv);

        xmin_LCOE =x[indicesLCOEmin_pv[1]];
        global minLCOH_pv , indicesLCOHmin_pv = findmin(LCOH_pv);
        global LCOH_min_pv  = minLCOH_pv;
        global xmin_LCOH_pv = x[indicesLCOHmin_pv[1]];
        global zmin_LCOH_pv = z_pv[indicesLCOHmin_pv[2]];

        delta_pv    = (endvalue_pv - startvalue_pv) / (d_pv - 1);
        delta_el_pv = (endvalue_el_pv - startvalue_el_pv) / (d_pv - 1);

        global startvalue_pv    = max(xmin_LCOH - delta_pv , 1);
        global endvalue_pv      = min(xmin_LCOH + delta_pv , max_cap_pv);
        global startvalue_el_pv = zmin_LCOH_pv - delta_el_pv;
        global endvalue_el_pv   = zmin_LCOH_pv + delta_el_pv;
        ...
        if l==length(dd_pv)
            global annual_hydrogen_production_final_pv = annual_hydrogen_production_pv
        end
    end

	rescaling_factor_pv = xmin_LCOH / max_cap_pv;

    x_min_LCOH_rescaled	            = xmin_LCOH / rescaling_factor_pv;
    z_min_LCOH_rescaled_pv          = zmin_LCOH_pv / rescaling_factor_pv;
    y_onshore_min_LCOH_rescaled_pv  = 0;
    y_offshore_min_LCOH_rescaled_pv = 0;

    rescaled_annual_hydrogen_production_pv = annual_hydrogen_production_final_pv[indicesLCOHmin_pv[1] , indicesLCOHmin_pv[2]] / rescaling_factor_pv;

    return x_min_LCOH_rescaled, y_onshore_min_LCOH_rescaled_pv, y_offshore_min_LCOH_rescaled_pv,
            z_min_LCOH_rescaled_pv, LCOH_min_pv, rescaled_annual_hydrogen_production_pv
end

function iterative_find_min_2var_onshore(loptions, CF_onshore, max_capacity_onshore, CAPEX_wind_onshore, CAPEX_el, WACC_el, WACC_onshore)

    lifetime_stack = 80000;              # Hours
    lifetime_el = 15;                    # Years
    lifetime_onshore = 25;                    # Years
    OPEX_el = 0.03;                      # %CAPEX
    OPEX_onshore = 0.015;
    eff_el = 0.7;
    min_CF_electrolyzer = 0.3;
    ...
    cost_onshore = (CAPEX_wind_onshore*WACC_onshore)/(1-(1+WACC_onshore)^(-lifetime_onshore))+(CAPEX_wind_onshore*OPEX_onshore);
    cost_el      = (CAPEX_el*WACC_el)/(1-(1+WACC_el)^(-lifetime_el))+(CAPEX_el*OPEX_el);

    global LCOH_min_onshore = 0;
    global xmin_LCOH        = 0;
    global max_cap_onshore  = max_capacity_onshore;
    global zmin_LCOH        = 0;

    global startvalue_onshore    = 0;
    global endvalue_onshore      = max_cap_onshore;
    global startvalue_el_onshore = 1;
    global endvalue_el_onshore   = max_cap_onshore;

    dd_onshore=(7,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4);

    for l in 1:length(dd_onshore)
        d_onshore=dd_onshore[l];

        y_onshore = collect(range(startvalue_onshore,    length=d_onshore, stop=endvalue_onshore));
        z_onshore = collect(range(startvalue_el_onshore, length=d_onshore, stop=endvalue_el_onshore));

        annual_invest_LCOE_onshore         = zeros(length(y_onshore));
        annual_energy_production_onshore   = zeros(length(y_onshore));
        annual_invest_LCOH_onshore         = zeros(length(y_onshore),length(z_onshore));
        annual_hydrogen_production_onshore = zeros(length(y_onshore),length(z_onshore));

        sumCF_onshore=sum(CF_onshore);

        for i in 1:length(y_onshore)
	        annual_invest_LCOE_onshore[i]       = cost_onshore  * y_onshore[i];
	        annual_energy_production_onshore[i] = sumCF_onshore * y_onshore[i];
		        for k in 1:length(z_onshore)
	                annual_invest_LCOH_onshore[i,k] = annual_invest_LCOE_onshore[i] + cost_el * z_onshore[k];
                        for t in 1:8760
                            annual_hydrogen_production_onshore[i,k] += min((CF_onshore[t]*y_onshore[i]),z_onshore[k]) / H2_energy_consumption;
                        end
                end
        end

        LCOE_onshore = 1000 * annual_invest_LCOE_onshore ./ annual_energy_production_onshore;
        LCOH_onshore = annual_invest_LCOH_onshore./annual_hydrogen_production_onshore;
        LCOE_min_onshore , indicesLCOEmin_onshore = findmin(LCOE_onshore);

        y_onshoremin_LCOE =y_onshore[indicesLCOEmin_onshore[1]];
        global minLCOH_onshore , indicesLCOHmin_onshore = findmin(LCOH_onshore);
        global LCOH_min_onshore  = minLCOH_onshore;
        global y_onshoremin_LCOH = y_onshore[indicesLCOHmin_onshore[1]];
        global zmin_LCOH_onshore = z_onshore[indicesLCOHmin_onshore[2]];

        delta_onshore    = (endvalue_onshore - startvalue_onshore) / (d_onshore - 1);
        delta_el_onshore = (endvalue_el_onshore - startvalue_el_onshore) / (d_onshore - 1);

        global startvalue_onshore    = max(y_onshoremin_LCOH - delta_onshore , 1);
        global endvalue_onshore      = min(y_onshoremin_LCOH + delta_onshore , max_cap_onshore);
        global startvalue_el_onshore = zmin_LCOH_onshore - delta_el_onshore;
        global endvalue_el_onshore   = zmin_LCOH_onshore + delta_el_onshore;

        if startvalue_onshore < 0
            startvalue_onshore = 0
        end

        if endvalue_onshore < 0
            endvalue_onshore = 0
        end

        if startvalue_el_onshore < 0
            startvalue_el_onshore = 0
        end

        if endvalue_el_onshore < 0
            endvalue_el_onshore = 0
        end

        if l==length(dd_onshore)
            global annual_hydrogen_production_final_onshore = annual_hydrogen_production_onshore
        end
    end

	rescaling_factor_onshore = y_onshoremin_LCOH / max_cap_onshore;

    y_onshore_min_LCOH_rescaled	         = y_onshoremin_LCOH / rescaling_factor_onshore;
    z_min_LCOH_rescaled_onshore          = zmin_LCOH_onshore / rescaling_factor_onshore;
    x_min_LCOH_rescaled_onshore          = 0;
    y_offshore_min_LCOH_rescaled_onshore = 0;

    rescaled_annual_hydrogen_production_onshore = annual_hydrogen_production_final_onshore[indicesLCOHmin_onshore[1] , indicesLCOHmin_onshore[2]] / rescaling_factor_onshore;

    return x_min_LCOH_rescaled_onshore , y_onshore_min_LCOH_rescaled, y_offshore_min_LCOH_rescaled_onshore,
            z_min_LCOH_rescaled_onshore, LCOH_min_onshore, rescaled_annual_hydrogen_production_onshore
end

function iterative_find_min_2var_offshore(loptions, CF_offshore, max_capacity_offshore, CAPEX_wind_offshore, CAPEX_el, WACC_el, WACC_offshore)

    lifetime_stack = 80000;              # Hours
    lifetime_el = 15;                    # Years
    lifetime_offshore = 25;                    # Years
    OPEX_el = 0.03;                      # %CAPEX
    OPEX_offshore = 0.015;
    eff_el = 0.7;
    min_CF_electrolyzer = 0.3;
    ...
    cost_offshore = (CAPEX_wind_offshore*WACC_offshore)/(1-(1+WACC_offshore)^(-lifetime_offshore))+(CAPEX_wind_offshore*OPEX_offshore);
    cost_el      = (CAPEX_el*WACC_el)/(1-(1+WACC_el)^(-lifetime_el))+(CAPEX_el*OPEX_el);

    global LCOH_min_offshore = 0;
    global xmin_LCOH         = 0;
    global max_cap_offshore  = max_capacity_offshore;
    global zmin_LCOH         = 0;

    global startvalue_offshore    = 0;
    global endvalue_offshore      = max_cap_offshore;
    global startvalue_el_offshore = 1;
    global endvalue_el_offshore   = max_cap_offshore;

    dd_offshore=(7,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4);

    for l in 1:length(dd_offshore)
        d_offshore=dd_offshore[l];

        y_offshore = collect(range(startvalue_offshore,    length=d_offshore, stop=endvalue_offshore));
        z_offshore = collect(range(startvalue_el_offshore, length=d_offshore, stop=endvalue_el_offshore));

        annual_invest_LCOE_offshore         = zeros(length(y_offshore));
        annual_energy_production_offshore   = zeros(length(y_offshore));
        annual_invest_LCOH_offshore         = zeros(length(y_offshore),length(z_offshore));
        annual_hydrogen_production_offshore = zeros(length(y_offshore),length(z_offshore));

        sumCF_offshore=sum(CF_offshore);

        for i in 1:length(y_offshore)
	        annual_invest_LCOE_offshore[i]       = cost_offshore  * y_offshore[i];
	        annual_energy_production_offshore[i] = sumCF_offshore * y_offshore[i];
		        for k in 1:length(z_offshore)
	                annual_invest_LCOH_offshore[i,k] = annual_invest_LCOE_offshore[i] + cost_el * z_offshore[k];
                        for t in 1:8760
                            annual_hydrogen_production_offshore[i,k] += min((CF_offshore[t]*y_offshore[i]),z_offshore[k]) / H2_energy_consumption;
                        end
                end
        end

        LCOE_offshore = 1000 * annual_invest_LCOE_offshore ./ annual_energy_production_offshore;
        LCOH_offshore = annual_invest_LCOH_offshore./annual_hydrogen_production_offshore;
        LCOE_min_offshore , indicesLCOEmin_offshore = findmin(LCOE_offshore);

        y_offshoremin_LCOE =y_offshore[indicesLCOEmin_offshore[1]];
        global minLCOH_offshore , indicesLCOHmin_offshore = findmin(LCOH_offshore);
        global LCOH_min_offshore  = minLCOH_offshore;
        global y_offshoremin_LCOH = y_offshore[indicesLCOHmin_offshore[1]];
        global zmin_LCOH_offshore = z_offshore[indicesLCOHmin_offshore[2]];

        delta_offshore    = (endvalue_offshore - startvalue_offshore) / (d_offshore - 1);
        delta_el_offshore = (endvalue_el_offshore - startvalue_el_offshore) / (d_offshore - 1);

        global startvalue_offshore    = max(y_offshoremin_LCOH - delta_offshore , 1);
        global endvalue_offshore      = min(y_offshoremin_LCOH + delta_offshore , max_cap_offshore);
        global startvalue_el_offshore = zmin_LCOH_offshore - delta_el_offshore;
        global endvalue_el_offshore   = zmin_LCOH_offshore + delta_el_offshore;

        if startvalue_offshore < 0
            startvalue_offshore = 0
        end

        if endvalue_offshore < 0
            endvalue_offshore = 0
        end

        if startvalue_el_offshore < 0
            startvalue_el_offshore = 0
        end

        if endvalue_el_offshore < 0
            endvalue_el_offshore = 0
        end

        if l==length(dd_offshore)
            global annual_hydrogen_production_final_offshore = annual_hydrogen_production_offshore
        end
    end

	rescaling_factor_offshore = y_offshoremin_LCOH / max_cap_offshore;

    y_offshore_min_LCOH_rescaled	     = y_offshoremin_LCOH / rescaling_factor_offshore;
    z_min_LCOH_rescaled_offshore         = zmin_LCOH_offshore / rescaling_factor_offshore;
    x_min_LCOH_rescaled_offshore         = 0;
    y_onshore_min_LCOH_rescaled_offshore = 0;

    rescaled_annual_hydrogen_production_offshore = annual_hydrogen_production_final_offshore[indicesLCOHmin_offshore[1] , indicesLCOHmin_offshore[2]] / rescaling_factor_offshore;

    return x_min_LCOH_rescaled_offshore , y_onshore_min_LCOH_rescaled_offshore, y_offshore_min_LCOH_rescaled,
            z_min_LCOH_rescaled_offshore, LCOH_min_offshore, rescaled_annual_hydrogen_production_offshore
end
