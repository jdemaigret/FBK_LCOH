using DelimitedFiles, StatsBase, FLoops
...
# (Col. 3)  -> WACC Onshore
# (Col. 4)  -> WACC Offshore
# (Col. 5)  -> WACC Utilities
# (Col. 6)  -> WACC equal 10%
# (Col. 7)  -> WACC 7.5%
#
# ---CAPEX 2050---
# (Col. 8)  -> Baseline PV 2050
# (Col. 9)  -> Baseline_Onshore 2050
# (Col. 10) -> Baseline_Offshore_1 2050
# (Col. 11) -> Baseline_Offshore_2 2050
# (Col. 12) -> Optimistic_PV 2050
# (Col. 13) -> Optimistic_Onshore 2050
# (Col. 14) -> Optimistic_Offshore_1 2050
# (Col. 15) -> Optimistic_Offshore_2 2050
#
# ---CAPEX 2021---
# (Col. 16) -> Baseline PV 2021
# (Col. 17) -> Baseline_Onshore 2021
# (Col. 18) -> Baseline_Offshore_1 2021
# (Col. 19) -> Baseline_Offshore_2 2021
# (Col. 20) -> Optimistic_PV 2021
# (Col. 21) -> Optimistic_Onshore 2021
# (Col. 22) -> Optimistic_Offshore_1 2021
# (Col. 23) -> Optimistic_Offshore_2 2021

function GIS_LCOH()

    array_CSV = readdlm("WACC_CAPEX Versione 2.csv", ',')
    # Choice between variable WACC as a function of technology/country
    WACC_value = "Variable_WACC"
    # WACC_value = 0.1

    for i in length(array_CSV[:,1])
        ...
        # Prende il nome del paese dal CSV
        region_string = array_CSV[i,1]
        # Creo la const e la salvo
        selected_region = [ region_string GADM(region_string) ]
        saveregions(region_string, selected_region)

        WACC_el             = 0.8;
        WACC_pv             = array_CSV[i,2];
        WACC_onshore        = array_CSV[i,3];
        WACC_offshore       = array_CSV[i,4];
        CAPEX_el            = 250;
        CAPEX_pv            = array_CSV[i,8];
        CAPEX_wind_onshore  = array_CSV[i,9];
        CAPEX_wind_offshore = array_CSV[i,10];

        function GISlcoh(gisregion=region_string, WACC_value, WACC_el, WACC_pv, WACC_onshore, WACC_offshore, CAPEX_el, CAPEX_pv, CAPEX_wind_onshore, CAPEX_wind_offshore; savetodisk=false, plotmasks=false, optionlist...)
            ...
            GTI_x_stampa, wind_onshore_x_stampa, wind_offshore_x_stampa = calc_LCOH_CAPS_PROD(loptions, regions, solarGTI, offshoreregions, regionlist, mask_pv_lcoh,
                                                                                    lonrange, latrange, windatlas, meanwind, windspeed, mask_onshore_lcoh, mask_offshore_lcoh,
                                                                                    meanGTI, WACC_value, WACC_el, WACC_pv, WACC_onshore, WACC_offshore, CAPEX_el, CAPEX_pv, CAPEX_wind_onshore, CAPEX_wind_offshore)

            vectorized_lcoh_matrix = vec(lcoh_region_matrix);
            vectorized_prod_matrix = vec(production_region_matrix);
            vectorized_lcoh_matrix_wo_0 = zeros(length(vectorized_lcoh_matrix))
            vectorized_prod_matrix_wo_0 = zeros(length(vectorized_prod_matrix))
            for s in 1:length(vectorized_lcoh_matrix)
                if vectorized_lcoh_matrix[s]==0 || vectorized_prod_matrix[s]==0
                    vectorized_lcoh_matrix[s] = 0
                    vectorized_prod_matrix[s] = 0
                elseif vectorized_lcoh_matrix[s]<0 || vectorized_prod_matrix[s]<0
                    vectorized_lcoh_matrix[s] = 0
                    vectorized_prod_matrix[s] = 0
                end
            end
            ...
            # supply_cost_data = readdlm("C:\\Users\\jdemaigret\\GISData\\output\\$gisregion supply_cost.txt", ',');
            # lcoh_vector = filter(!iszero, supply_cost_data[:,1]);
            # prod_vector = filter(!iszero, supply_cost_data[:,2]);
            #scene = Makie.lines(lcoh_prod_array_sorted[:,2], lcoh_prod_array_sorted[:,1], color = :black)
            #println("...saving data plot (lines)...");
            #mkpath(in_datafolder("output"))
            #filename = in_datafolder("output", "$gisregion supply_cost.png")
            ...
            # el_region_matrix,
            # lcoh_region_matrix,
            # production_region_matrix,
            # PV_pv_onshore_erares = calc_LCOH_CAPS_PROD(loptions, regions, solarGTI, offshoreregions, regionlist, mask_pv_lcoh,
            #                     lonrange, latrange, windatlas, meanwind, windspeed, mask_onshore_lcoh, mask_offshore_lcoh,
            #                     meanGTI)
            ...
            # #
            # plot_GTI(loptions, regions, mask_lcoh, mask_offshore_lcoh, lonrange, latrange, GTI_x_stampa, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)
            # plot_wind_onshore(loptions, regions, mask_lcoh, mask_offshore_lcoh, lonrange, latrange, wind_onshore_x_stampa, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)
            # plot_wind_offshore(loptions, regions, mask_lcoh, mask_offshore_lcoh, lonrange, latrange, wind_offshore_x_stampa, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)
        ...
        function plot_GTI(loptions, regions, mask_lcoh, mask_offshore_lcoh, lonrange, latrange, GTI_x_stampa, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)

            @unpack gisregion, filenamesuffix = loptions

            GTI_x_stampa_res = rescale_property(GTI_x_stampa,  loptions,  lonrange , latrange, regionlist, solarGTI, regions)
            ...
            if plotmasks != false   # can == :onlymasks as well
                ...
                to_plot = GTI_x_stampa_res;       #Quello che voglio plottare
                to_mask = ones(size(regions))*4;     #Quello che voglio mascherare
                ...
                scene = createmap("$(gisregion)_masks_GTI_x_stampa$filenamesuffix", to_plot, to_mask, regionlist, lons, lats, [], source, dest, xs, ys,
                     [], [], [], [], lines=false, labels=false, resolutionscale=1, textscale=1, legend=false)
            end
            nothing
        end
        function plot_wind_onshore(loptions, regions, mask_lcoh, mask_offshore_lcoh, lonrange, latrange, wind_onshore_x_stampa, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)

            @unpack gisregion, filenamesuffix = loptions

            wind_onshore_x_stampa_res = rescale_property(wind_onshore_x_stampa,  loptions,  lonrange , latrange, regionlist, solarGTI, regions)
            ...
            if plotmasks != false   # can == :onlymasks as well
                ...
                to_plot = wind_onshore_x_stampa_res;       #Quello che voglio plottare
                to_mask = ones(size(regions))*4;     #Quello che voglio mascherare
                ...
                scene = createmap("$(gisregion)_wind_onshore_x_stampa$filenamesuffix", to_plot, to_mask, regionlist, lons, lats, [], source, dest, xs, ys,
                     [], [], [], [], lines=false, labels=false, resolutionscale=1, textscale=1, legend=false)
            end
            nothing
        end
        function plot_wind_offshore(loptions, regions, mask_lcoh, mask_offshore_lcoh, lonrange, latrange, wind_offshore_x_stampa, regionlist, aqueduct, slope, solarGTI; plotmasks=plotmasks, downsample=1)

            @unpack gisregion, filenamesuffix = loptions

            wind_offshore_x_stampa_res = rescale_property(wind_offshore_x_stampa,  loptions,  lonrange , latrange, regionlist, solarGTI, regions)
            ...
            if plotmasks != false   # can == :onlymasks as well
                ...
                to_plot = wind_offshore_x_stampa_res;       #Quello che voglio plottare
                to_mask = ones(size(regions))*4;     #Quello che voglio mascherare
                ...
                scene = createmap("$(gisregion)_wind_offshore_x_stampa$filenamesuffix", to_plot, to_mask, regionlist, lons, lats, [], source, dest, xs, ys,
                     [], [], [], [], lines=false, labels=false, resolutionscale=1, textscale=1, legend=false)
            end
            nothing
        end
        ...
        function calc_LCOH_CAPS_PROD(loptions, regions, solarGTI, offshoreregions, regionlist, mask_pv_lcoh,
                                    lonrange, latrange, windatlas, meanwind, windspeed, mask_onshore_lcoh, mask_offshore_lcoh,
                                    meanGTI, WACC_value, WACC_el, WACC_pv, WACC_onshore, WACC_offshore,
                                    CAPEX_el, CAPEX_pv, CAPEX_wind_onshore, CAPEX_wind_offshore)
            ...
            # Matrice alta risoluzione
                ...
                GTI_x_stampa            = reshape(sum(solarGTI, dims=1), (nlons, nlats));
                wind_onshore_x_stampa   = reshape(sum(speed2capacityfactor_onshore.(windspeed), dims=1), (nlons, nlats));
                wind_offshore_x_stampa  = reshape(sum(speed2capacityfactor_offshore.(windspeed), dims=1), (nlons, nlats));
            ...
            #for j in 2:4
            #@floop ThreadedEx()
            @time   @floop ThreadedEx() for j in 1:nlats
                        eralat = eralats[j]
                        colrange = latmap[lat2col(eralat+erares/2, res):lat2col(eralat-erares/2, res)-1]
                        # println("colrange:")
                        # println(colrange)
                        ...
                        for i in 1:nlons
                            ...
                            PROD_pv_onshore_erares[i,j] = find_minimum_LCOH(loptions, lonrange, latrange, regionlist, dim_year_lon_lats,
                                                            GTI, area, wind_onshore, wind_offshore, 1, WACC_value,
                                                            WACC_el, WACC_pv, WACC_onshore, WACC_offshore,
                                                            CAPEX_el, CAPEX_pv, CAPEX_wind_onshore, CAPEX_wind_offshore); # Il numero sarebbe il type_min = 1
                            PV_pv_erares[i,j],
                            ...
                            PROD_pv_erares[i,j]         = find_minimum_LCOH(loptions, lonrange, latrange, regionlist, dim_year_lon_lats,
                                                            GTI, area, wind_onshore, wind_offshore, 2, WACC_value
                                                            WACC_el, WACC_pv, WACC_onshore, WACC_offshore,
                                                            CAPEX_el, CAPEX_pv, CAPEX_wind_onshore, CAPEX_wind_offshore); # Il numero sarebbe il type_min  = 2
                            ...
                            LCOH_onshore_erares[i,j]    = find_minimum_LCOH(loptions, lonrange, latrange, regionlist, dim_year_lon_lats,
                                                            GTI, area, wind_onshore, wind_offshore, 3, WACC_value
                                                            WACC_el, WACC_pv, WACC_onshore, WACC_offshore,
                                                            CAPEX_el, CAPEX_pv, CAPEX_wind_onshore, CAPEX_wind_offshore); # Il numero sarebbe il type_min  = 3
                            ...
                            LCOH_offshore_erares[i,j]   = find_minimum_LCOH(loptions, lonrange, latrange, regionlist, dim_year_lon_lats,
                                                            GTI, area, wind_onshore, wind_offshore, 4, WACC_value
                                                            WACC_el, WACC_pv, WACC_onshore, WACC_offshore,
                                                            CAPEX_el, CAPEX_pv, CAPEX_wind_onshore, CAPEX_wind_offshore); # Il numero sarebbe il type_min  = 4
            ...
            return pv_region_matrix, onshore_region_matrix, offshore_region_matrix, el_region_matrix, lcoh_region_matrix, production_region_matrix,
                    ...
                    PV_onshore_erares, ONSHORE_onshore_erares, OFFSHORE_onshore_erares, EL_onshore_erares, LCOH_onshore_erares, PROD_onshore_erares,
                    PV_offshore_erares, ONSHORE_offshore_erares, OFFSHORE_offshore_erares, EL_offshore_erares, LCOH_offshore_erares, PROD_offshore_erares,
                    GTI_x_stampa, wind_onshore_x_stampa, wind_offshore_x_stampa
        end
        ...
        function find_minimum_LCOH(loptions, lonrange, latrange, regionlist, dim_year_lon_lats,
            ...
            # indice_mio = findfirst(isequal(D), vettori_coi_nomi)
            # WACC_mio = matrice_con_tutto[indice_mio,2]

            CAPEX_el               = CAPEX_el;
            CAPEX_pv               = CAPEX_pv;
            CAPEX_wind_onshore     = CAPEX_wind_onshore;
            CAPEX_wind_offshore    = CAPEX_wind_offshore;
            WACC_el                = WACC_el


            if WACC_value == "Variable_WACC"
                WACC_pv       = WACC_pv
                WACC_onshore  = WACC_onshore
                WACC_offshore = WACC_offshore
            else
                WACC_pv       = WACC_value
