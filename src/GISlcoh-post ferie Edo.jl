...
lcohoptions() = Dict(
    ...
    :plant_area => 1,#0.05,                 # area available for PV or CSP plants after the masks have been applied
    ...
    :area_onshore => 1,#0.08,               # area available for onshore wind power after the masks have been applied
    :area_offshore => 1,#0.33,              # area available for offshore wind power after the masks have been applied
...
function GISlcoh(; savetodisk=false, plotmasks=false, optionlist...)
    ...
    mask_pv_lcoh = create_solar_mask_lcoh(loptions, regions, popdens, land, protected, lonrange, latrange, mask_altitude_lcoh, mask_aqueduct_lcoh; plotmasks=plotmasks, downsample=1)
    ...
    mask_onshore_lcoh,
    mask_offshore_lcoh = create_wind_masks_lcoh(loptions, regions, offshoreregions, gridaccess, popdens, topo, land, protected, lonrange, latrange,mask_altitude_lcoh, mask_aqueduct_lcoh; plotmasks=plotmasks, downsample=downsample_masks)
    ...
    calculate_areas(loptions, regions, solarGTI, lonrange, latrange, mask_pv_lcoh, mask_onshore_lcoh, mask_offshore_lcoh)
    ...
    plot_lcoh(loptions, regions, mask_lcoh, mask_offshore_lcoh, lonrange, latrange, lcoh_region_matrix, regionlist, aqueduct, slope, mask_altitude_lcoh, mask_aqueduct_lcoh, mask_pv_lcoh, mask_onshore_lcoh; plotmasks=plotmasks, downsample=1)
...
#Nuova funzione
function create_solar_mask_lcoh(loptions, regions, popdens, land, protected, lonrange, latrange, mask_altitude_lcoh, mask_aqueduct_lcoh; plotmasks=false, downsample=1)
    ...
    mask_pv_lcoh = (popdens .< pv_plant_persons_per_km2) .& goodland .& .!pv_protected_area  .& mask_altitude_lcoh .& mask_aqueduct_lcoh

    return mask_pv_lcoh
end

#Nuova funzione
function create_wind_masks_lcoh(loptions, regions, offshoreregions, gridaccess, popdens, topo, land, protected, lonrange, latrange, mask_altitude_lcoh, mask_aqueduct_lcoh; plotmasks=false, downsample=1)
    ...
    mask_onshore_lcoh = (popdens .< wind_persons_per_km2) .& goodland .& .!wind_protected_area .& mask_altitude_lcoh .& mask_aqueduct_lcoh
...
#Nuova funzione
function plot_lcoh(loptions, regions, mask_lcoh, mask_offshore_lcoh, lonrange, latrange, lcoh_region_matrix, regionlist, aqueduct, shore, mask_altitude_lcoh, mask_aqueduct_lcoh, mask_pv_lcoh, mask_onshore_lcoh; plotmasks=plotmasks, downsample=1)
...
function calculate_areas(loptions, regions, solarGTI, lonrange, latrange, mask_pv_lcoh, mask_onshore_lcoh, mask_offshore_lcoh)
    ...
    global area_tot = 0;
    global installable_hybrid_pv=0;
    global installable_hybrid_onshore=0;
            ...
            for c in colrange
                for r in rowrange
                    ...
                    if hybrid_system_mask[r,c]>0 #mask_pv_lcoh[r,c]>0 && mask_onshore_lcoh[r,c]>0
                        area_idonea_pv_onshore += (0.5*(area*plant_area))+(0.5*(area*area_onshore))
                        installable_hybrid_pv += 0.5*area*plant_area*pv_density/1000000
                        installable_hybrid_onshore += 0.5*area*area_onshore*onshore_density/1000000
                    end
                    if regions[r,c]>0
                        area_tot += area
                    end
    ...
    installable_pv             = (area_idonea_pv         * pv_density)    / 1000000;
    installable_onshore        = (area_idonea_onshore    * onshore_density)  / 1000000;
    installable_offshore       = (area_idonea_offshore   * offshore_density) / 1000000;
    #installable_hybrid_pv      = 0.5*(area_idonea_pv_onshore * pv_density       * plant_area)    / 1000000;
    #installable_hybrid_onshore = 0.5*(area_idonea_pv_onshore * onshore_density  * area_onshore)  / 1000000;
