...
function annualwindindex(Woption; resource=:wind, sites_quantile=1.0, aggregateregions=[])
    @unpack gisregion, era_year, res, erares, filenamesuffix = Woption
...
function annualwindindex(; resource=:wind, sites_quantile=1.0, aggregateregions=[], optionlist...)
    Woption = WindOptions(merge(windoptions(), optionlist))
    return annualwindindex(Woption, resource=resource, sites_quantile=sites_quantile,
